...
class Audio extends CI_Controller {
	...
	function __construct()
	{
		...
		$this->load->model('audiomodel','',TRUE);
	...
	public function index($limit='',$page='')
	{
        ...
        if($_POST){
         ...
         if ( ! $this->upload->do_upload('audiolink')) {
        ...
        //   $audioLink= $data['upload_data']['full_path'];
        $audioLink= $data['upload_data']['file_name'];
        
		    $this->audiomodel->SaveAudio($audioLink);	
         } 	
...
redirect('audio');
		 ...
		 $data['total']=$this->audiomodel->_get_all();
		$data['per_page'] = $limit;
		/**configration of pagination***/
		$config["base_url"] = base_url() . "audio/index/".$limit."/";
		$config['total_rows'] = $data['total'];
		$config["per_page"] = $limit;
		$this->pagination->initialize($config);


		$blacklist = $this->audiomodel->ShowAudio($config['per_page'],$page);
		$data["links"] = $this->pagination->create_links();
 		$data['audio']=$blacklist;
 		//$this->layout->view('blacklisting',$data);

		$this->load->view('layout/header',$data);
		$this->load->view('audio/home',$data);
	}

	function deleteAudio(){
	$this->audiomodel->deleteaudio();
}

function getaudios(){
	    $OrgIDKey = $this->input->get('key');
	     $organizationID= $this->audiomodel->get_orgId_by_Key($OrgIDKey);

         $result = $this->audiomodel->getaudios($organizationID);
